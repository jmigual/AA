...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Function{GEOGRAPHY\_REPEAT}{$G = (V, E), v, n$}
			\If{$n - 1 \ge |V|$}
				\State \Return \texttt{"es\_empat"}
			\ElsIf{$\exists w | (v,w) \in E \ $ \Call{GEOGRAPHY\_REPEAT}{$G$, $w$, $n+1$} \texttt{== "es\_guanyador"}}
				\State \Return \texttt{"es\_perdedor"}
			\ElsIf{$\exists w| (v, w) \in E \ $ \Call{GEOGRAPHY\_REPEAT}{$G$, $w$, $n+1$} \texttt{== "es\_empat"}}
				\State \Return \texttt{"es\_empat"}
			\Else
